...
export class Card {
  constructor(dataCard, templateCard, openImagePopup, handleDelete, getId, likeCardApi, dislikeCardApi) {
    ...
    this._isLiked = dataCard.likes.length != 0 ? dataCard.likes.find(item => item._id == this._getId()) : false
  }
  ...
  _setEventListeners() {
    ...
    this._deleteButton.addEventListener("click", () => this._handleDeleteCard(this._dataCard, this._handleDelete.bind(this)));
    this._likeButton.addEventListener("click", ()=> this._changeLikeApi(this._dataCard));
  }
  ...
  _getElements() {
    ...
    this._isLiked ? this._likeButton.classList.add("card__like-image_active") : null
  }
  ...
  create() {
    ...
    this.getLikesCount(this._dataCard)
